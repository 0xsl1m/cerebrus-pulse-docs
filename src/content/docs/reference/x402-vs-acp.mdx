...
| **Payment** | USDC micropayments on Base | USDC via x402 on Base | VIRTUAL token escrow |
...
| **Settlement** | Instant (Base L2) | Instant (Base L2) | ACP escrow release |
...
x402 and A2A share the same USDC payment rails — A2A adds agent discovery and task semantics on top. ACP provides AI synthesis with multi-model reasoning.

## Migration

Moving between protocols doesn't require account changes. x402 and A2A both use wallet-based USDC payments on Base. ACP uses VIRTUAL tokens on Virtuals Protocol. There's no lock-in to any protocol.
